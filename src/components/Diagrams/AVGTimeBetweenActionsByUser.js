...
export default class AvgTimeBetweenActionsByUserDiagram extends CommonDiagram {
  @autobind drawChart() {
    ...
    $(this.container).highcharts({
      ...
      tooltip: {
        formatter: function () {
          ...
          return `<span style="font-size: 10px">${this.series.name}</span> <br />` +
            `<span style="color:${this.point.color}">\u25CF </span>${this.point.name}: <b>${avgLabels[this.point.y]}</b> <br />`;
        }
      },
      ...
      series: [{
        ...
        data: formattedData.length ? formattedData && formattedData.map(({ full_name, avg_in_hours }) => [full_name, avg_in_hours]) : []
      }]
    });
  }
}
